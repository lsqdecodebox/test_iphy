...
class KMeans(object):
    ...
    def quantize(self):
        ...
        ntot = np.array([np.sum(self._KMeans.labels_ == i) for i in range(self.maxColor)])
        id_sort = np.argsort(-ntot)
        # print(id_sort, ntot, ntot.sum(), ntot[id_sort])
        theme = np.array(self._KMeans.cluster_centers_, dtype=np.uint8)
        # print(theme, theme[id_sort])
        return theme[id_sort], ntot[id_sort]
